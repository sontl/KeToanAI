...
import 'package:ketoan_ai/constants/api_consts.dart';
import 'package:ketoan_ai/models/chat_model.dart';
import 'package:ketoan_ai/models/models_model.dart';
...
class ApiService {
  ...
  static Future<List<ChatModel>> sendMessageClaude(
    ...
    try {
      log("modelId $modelId");
      var response = await http.post(
        Uri.parse("$ANTHROPIC_BASE_URL/messages"),
        headers: {
      ...
      if (jsonResponse["content"].length > 0) {
        ...
        chatList = List.generate(
          ...
          (index) => ChatModel(
            msg: jsonResponse["content"][index]["text"],
            chatIndex: 1,
          ),
        );
      }
